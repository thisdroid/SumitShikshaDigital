...
import styles from './AddCollegeQuestions.module.css';
import { Helmet } from 'react-helmet-async';

const AddCollegeQuestions = () => {
  ...
  useEffect(() => {
    ...
    setExams(allExams.filter((e) => e.examType === 'college'));
  }, []);
  ...
  const handleSubmit = (e) => {
    ...
    const allExams = JSON.parse(localStorage.getItem('exams') || '[]');
    const examIndex = allExams.findIndex((e) => e.examName === examName && e.examType === 'college');

    if (examIndex === -1) {
      alert(`Exam not found: ${examName}`);
      return;
    }

    if (!allExams[examIndex].questions) {
      allExams[examIndex].questions = [];
    }

    allExams[examIndex].questions.push(
      ...questions.map((q) => ({
        question: q.question,
  ...
  return (
    <div className={styles.container}>
      <Helmet>
        ...
        <meta name="description" content="Create multiple choice questions for college exams" />
      </Helmet>
...
export default AddCollegeQuestions;
